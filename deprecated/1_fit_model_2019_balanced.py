...
model_name        = 'Xception_multi-class_classifier_2019_balanced_0' #how do we save the model when it's done

data_dir          = './data/processed/2019_challenge/' # where did we store the images
target_shape      = (150, 200) #image shape after re-sizing
target_size       = 6000 #how many cases per class after re-sampling
n_epochs_train    = 50
...
train_ds = make_balanced_dataset_from_image_directory(data_dir, batch_size=batch_size, target_size=target_size)
...
target_size = 1000
test_ds = make_balanced_dataset_from_image_directory(new_data_dir, batch_size=batch_size, target_size=target_size)
