...
def make_balanced_split_dataset_from_image_directory(base_dir, batch_size, target_size, split=[0.7, 0.1, 0.2], validation=True, shuffle=True):
    ...
    for label, sub_dir in enumerate(sub_dirs):  
        ...
        if validation:
            ...
            if label == 0:  
                train_ds, val_ds, test_ds = make_balanced_split_dataset_from_image_sub_directory(sub_dir, label, target_size, split, validation)
            else:
                sub_train_ds, sub_val_ds, sub_test_ds = make_balanced_split_dataset_from_image_sub_directory(sub_dir, label, target_size, split, validation)
            ...
            if label == 0:  
                train_ds, test_ds = make_balanced_split_dataset_from_image_sub_directory(sub_dir, label, target_size, split, validation)
            else:
                sub_train_ds, sub_val_ds, sub_test_ds = make_balanced_split_dataset_from_image_sub_directory(sub_dir, label, target_size, split, validation)
                train_ds = train_ds.concatenate(sub_train_ds)
                test_ds  = test_ds.concatenate(sub_test_ds)
